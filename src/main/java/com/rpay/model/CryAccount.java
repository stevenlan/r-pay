...
public class CryAccount extends Model<CryAccount> {
    ...
    @NotBlank( message = "{cryAcc.cryCode.empty}",groups = {AdminGroup.class,UserGroup.class})
    private String cryCode ;
    @ApiParam(name = "cryName", value = "别名，用户输入时必填", required = false)
    @NotBlank( message = "{cryAcc.cryName.empty}",groups = {UserGroup.class})
    private String cryName ;
    @ApiParam(name = "agreement", value = "加密链，目前只支持ERC或者TRC", required = true)
    @NotBlank( message = "{cryAcc.agreement.empty}",groups = {UserGroup.class})
    private String agreement ;
    @ApiParam(name = "cryAdd", value = "钱包地址", required = true)
    @NotBlank( message = "{cryAcc.cryAdd.empty}",groups = {UserGroup.class})
    private String cryAdd ;
    @ApiParam(name = "userId", value = "添加给哪个用户", required = true)
    @NotNull( message = "{cryAcc.user.empty}",groups = {AdminGroup.class})
    private Long userId ;
